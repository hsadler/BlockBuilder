...
public class PowerBlockScript : BaseBlockScript {
    ...
    private void PowerForwardConnectedBlock() {
        Vector3 neighborCoords = transform.position + (transform.rotation * Vector3.forward);
        if(BlockManager.instance.BlockExists(neighborCoords)) {
            GameObject connectedBlock = BlockManager.instance.GetBlock(neighborCoords);
            BaseBlockScript connectedBlockScript = connectedBlock.GetComponent<BaseBlockScript>();
